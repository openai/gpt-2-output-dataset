...
d_test_gpt2 <- fread("plot/webtext.test.model=gpt2.freq_power_1k.csv")
d_test_medium <- fread("plot/webtext.test.model=gpt2-medium.freq_power_1k.csv")
d_test_large <- fread("plot/webtext.test.model=gpt2-large.freq_power_1k.csv")
d_test_xl <- fread("plot/webtext.test.model=gpt2-xl.freq_power_1k.csv")

d_test_gpt2$model = "gpt2"
d_test_medium$model = "gpt2-medium"
d_test_large$model = "gpt2-large"
d_test_xl$model = "gpt2-xl"

d_test_c <- rbindlist(list(d_test_gpt2, d_test_medium, d_test_large, d_test_xl))
p_test_c <- ggplot(d_test_c, aes(freq, power)) +
  geom_smooth(aes(linetype = model, fill = model, colour = model))
ggsave("plot/webtext.test.4models.freq_power_1k.pdf", plot=p_test_c)

# small-117M.test
d_small_gpt2 <- fread("plot/small-117M.test.model=gpt2.freq_power_1k.csv")
d_small_medium <- fread("plot/small-117M.test.model=gpt2-medium.freq_power_1k.csv")
d_small_large <- fread("plot/small-117M.test.model=gpt2-large.freq_power_1k.csv")
d_small_xl <- fread("plot/small-117M.test.model=gpt2-xl.freq_power_1k.csv")
...
p_small_c <- ggplot(d_small_c, aes(freq, power)) +
  geom_smooth(aes(linetype = model, fill = model, colour = model))
ggsave("plot/small-117M.test.4models.freq_power_1k.pdf", plot=p_small_c)

# small-117-k40.test
d_small_k40_gpt2 <- fread("plot/small-117M-k40.test.model=gpt2.freq_power_1k.csv")
d_small_k40_medium <- fread("plot/small-117M-k40.test.model=gpt2-medium.freq_power_1k.csv")
d_small_k40_large <- fread("plot/small-117M-k40.test.model=gpt2-large.freq_power_1k.csv")
d_small_k40_xl <- fread("plot/small-117M-k40.test.model=gpt2-xl.freq_power_1k.csv")
